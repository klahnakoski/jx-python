    ...
    def to_python(self, loop_depth=0):
        ...
        if len(self.terms) == 1 and self.terms[0].name == ".":
            ...
            if self.terms[0].value.op == Variable.op and self.terms[0].value.var == "row":
                ...
                # select property
                source = f"""[{selects[0].value.source} for rows{loop_depth} in [{frum.source}] for rownum{loop_depth}, row{loop_depth} in enumerate(rows{loop_depth})]"""
        else:
            # structure selection
            select_sources = ",".join(quote(s.name) + ":" + s.value.source for s in selects)
            source = f"""[leaves_to_data({{{select_sources}}}) for rows{loop_depth} in [{frum.source}] for rownum{loop_depth}, row{loop_depth} in enumerate(rows{loop_depth})]"""

        return PythonScript(
            merge_locals(
                [s.value.locals for s in selects], frum.locals, leaves_to_data=leaves_to_data, delist=delist,
            ),
            loop_depth,
            array_of(self.type),
            source,
